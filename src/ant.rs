...
// - FIX SLope calculation errror when ant is at y=0
    ...
    pub fn found_food(&mut self, row: &u32, col: &u32) {
        ...
        fn is_near_home(&self) -> bool {
            let home_tolerance = 5; // Define a tolerance range around home
            (self.pos.0 as i32 - self.home.0 as i32).abs() <= home_tolerance &&
            (self.pos.1 as i32 - self.home.1 as i32).abs() <= home_tolerance
        }
